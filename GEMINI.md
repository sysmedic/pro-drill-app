...
이 프로젝트는 볼링 지공사가 모바일/PWA 환경에서 고객별 지공 차트, 작업 기록, 메모를 온라인(파이어베이스/Supabase)으로 관리하는 React 앱이다. AI가 컨텍스트를 잃어도 아래 문서와 명령을 기준으로 복구한다.
...
- 데이터베이스(Firestore/Supabase) 컬렉션 및 테이블 구조 변경은 특히 조심한다. 서비스 데이터가 깨질 수 있다.
