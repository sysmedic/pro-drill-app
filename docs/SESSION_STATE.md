...
- **신규 분기 브랜치 `드릴링-자동계산` [D-Pad 우측 `[ ⌖ AUTO FIT ]` 기능 및 3대 기준 홀 안전 잠금 엔진] 탑재 완료 (지공사님 지침에 따라 Vercel 배포는 보류 상태) (`Midline2DLayoutRenderer.jsx`)**:
  - **지공사님 핵심 지침 100% 완수**:
    1) **디패드 우측 `[ ⌖ AUTO FIT ]` 버튼 탑재**: 세련된 타겟 크로스헤어(`⌖`) 아이콘과 골드/앰버 테마 버튼을 풀스크린 및 인라인 D-Pad 우측에 배치.
    2) **3대 기준 홀(#1, #2, 원홀) 절대 잠금 안전장치(Safety Lock)**: #1, #2, 원홀 선택 시 `AUTO FIT` 버튼이 스텔스 비활성화(`opacity-30`, `cursor-not-allowed`)되어 오발 기준선 왜곡 100% 원천 차단.
    3) **곡률 기반 최대 내접 직경(1/64" 무조건 내림) 및 최적 이동거리(Y*) 동시 연산**: 중간 비트(#3, #4, #5 등) 선택 시 오발선을 0.001mm도 벗어나지 않는 안전한 표준 비트 규격과 1:1 완벽 밀착 좌표를 원클릭으로 계산 및 자동 안착.
