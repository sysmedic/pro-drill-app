...
- **미입력 스펙 자동 기본값(Default Fallback) 대입 및 사유 자동 명시 개발 완료**:
  ...
  2) 누락된 수치가 대체 적용되었을 경우, AI 추천 사유(`description`) 및 지공의도 기록(`intentSummary` 및 `reason`) 뒤에 **`(미입력 스펙: RPM(300), PAP(5" Right 1" Up)... 표준값 대체 적용)`** 형태의 안내 문구가 자동으로 함께 명시되도록 보완 완료.
