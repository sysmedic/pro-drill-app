...
- **신규 분기 브랜치 `드릴링-자동계산` [정밀도 3단계 (기본 3 / 정밀 5 / 초정밀 7) & 최대 8드릴(#8) 확장 & 실가공거리(L) 기반 자동추천 뱃지 및 부적격 경고 안내 연동 완성] (지공사님 지침에 따라 Vercel 배포는 보류 상태) (`OvalCalculatorView.jsx`, `OvalResultModal.jsx`, `Midline2DLayoutRenderer.jsx`, `SelectField.jsx`)**:
  - **지공사님 핵심 지침 100% 완수**:
    1) **정밀도 3단계 모드 & 7:3 비율 상단 바 UI**: `[ 기본 (3) ]` `[ 정밀 (5) ]` `[ 초정밀 (7) ]` 3개 버튼 탭과 보정 셀렉트를 7:3 비율로 깔끔하게 배치.
    2) **초정밀(7드릴) 순차 가공 이동 동선 배치**:
       - #1(상단 끝), #2(하단 끝), 원홀(중앙 마스터) 고정
       - **하단 연계 가공**: #3(하단 2/3), #4(하단 1/3) 자동 내접 계산 및 안착
       - **상단 연계 가공**: #5(상단 1/3), #6(상단 2/3) 자동 내접 계산 및 안착
       - #7: 원홀 마스터
    3) **피치 메트릭스 & 보정치 UI 완성**:
       - `(원홀)` 문구 삭제 및 드릴번호/비트크기 검정색(`text-slate-900`) 가독성 확보.
       - 보정치 셀렉트: 팝업에는 순수 수치(`0`, `1/128`, `1/64`, `3/128`, `1/32`) 표시, 선택 완료 시 `보정` / `1/128 보정` 형태 노출.
       - 셀렉트 포커스 링 외곽선 하이라이트 제거 및 모드 버튼 간 미세 구분선 적용.
    4) **오토핏(AUTO FIT) 기능 및 D-패드 버튼 완전 삭제**:
       - 3/5/7드릴 수학적 자동 내접 연산 체계 완성으로 불필요해진 레거시 오토핏 연산 함수 및 D-패드 내 `[ ⌖ AUTO FIT ]` 버튼 완전 제거하여 조작부 심플화.
    5) **실제 가공 이동거리($L_{\max}$) 기반 추천 뱃지 & 부적격 경고 배너 연동**:
       - **추천 뱃지(별모양) 위치의 스펙트럼 동적 배치**: 가공 이동거리($L_{\max}$)에 따라 기본 중앙(`left: 17%`), 기본~정밀 중간(`left: 33%`), 정밀 중앙(`left: 50%`), 정밀~초정밀 중간(`left: 67%`), 초정밀 중앙(`left: 83%`)으로 3버튼 트랙 상단을 부드럽게 실시간 이동하며 가공 난이도를 직관적으로 시각화.
       - **스판 변환 결과 모달 분수 단위 라디오 버튼 그룹 적용**:
         * 모달 상단 헤더의 분수 선택 방식을 버튼 토글에서 **깔끔한 원형 인디고 라디오 버튼(`◉ 16분`, `○ 32분`, `○ .5/16분`)**으로 변경하여 직관적인 단일 선택 UI 완성.
       - **미드라인 마킹 결과 모달 분수 단위 3-Way 토글(`[16분] [32분] [.5/16분]`) 증설**:
         * 기존 2단 버튼에서 지공 현장 실무에 최적화된 `[.5/16분]`(16분 분모 + 0.5단위 분자)을 추가하여 3단 토글로 확장.
         * 스판 변환 결과 모달과 100% 동일한 서체, R값(`rounded-md`) 및 통일된 버튼 라벨(`[.5/16분]`) 적용.
       - **아카이브(ARCHIVE) 모달 UI 간소화 및 안전 삭제 경고 모달 탑재**:
         * 저장 탭 상단 `슬롯 이름 / 메모` 불필요 라벨 삭제로 깔끔한 입력창 구성.
         * 하단 공유 버튼 텍스트: `현재 제원 즉시 공유` ➔ **`공유`**로 직관적 간소화 및 폰트 크기 동등화(`text-sm font-bold`).
         * **제원 삭제 확인 경고 모달 탑재 및 문구 정돈**: 타이틀 `아카이브 삭제`, 본문 `[기록이름] 기록이 영구적으로 삭제됩니다.`로 정돈.
       - **2D 시뮬레이터 에딧 모드 기본 방향 및 화살표 버튼 배치 순서 정비**:
         * 도면 화살표 기본값을 **하향 화살표 (`▼`, 아래쪽 방향)**으로 유지/복원.
         * 상단 툴바 화살표 버튼 배치는 **위쪽(`▲`) ➔ 아래쪽(`▼`) 순서**로 정렬 유지.
       - **오발 결과 모달 RE SET 버튼 및 확인 팝업 문구 정비**:
         * 버튼 명칭: `리셋` ➔ **`RE SET`**으로 변경.
         * 확인 팝업 안내 문구: *"모든 이동/추가 내역이 취소되고, 오발 피치 메트릭스 추천 상태로 복원됩니다."*로 정돈.
       - **앱 내 모든 버튼 디자인 R값 통일 (`rounded-md`, 둥글게 각진 형태)**:
         * 헤더 ARCHIVE 버튼, 3탭 스위처 버튼, 키패드 버튼, 모달 닫기/확인/리셋/저장/공유/삭제 버튼 등 앱 내 전체 버튼의 모서리 둥글기를 일관되게 `rounded-md`로 통일 적용.
       - **플로팅 말풍선 꼬리표 경계선 완벽 제거**:
         * 말풍선 상단 삼각 화살표 꼬리표와 말풍선 본체 사이의 가로 경계선을 `z-10` 마스킹으로 매끄럽게 덮어 100% 매끄러운 일체형 말풍선으로 완성.
       - **수치 초기화 버튼 디자인 슬레이트 계열 통일 & 3개 계산기 동일 규격 우측 상단 배치**:
         * 아카이브(ARCHIVE) 옆 전체 초기화 버튼 완전 삭제하여 헤더부 심플화.
         * 스판 변환기 및 미드라인 마킹 상단의 `손방향` 라벨 타이틀 삭제.
         * 스판 변환기, 미드라인 마킹, 오발 계산기 3개 탭 모두 손방향 토글 우측 상단 끝단에 동일한 슬레이트 규격의 `[수치 초기화]` 버튼을 일관되게 배치.
       - **미드라인 브릿지(Bridge) 옵션에 `5/16` 추가**: 주요 지정 간격 목록에 `5/16`을 증설하여 `["1/8", "3/16", "1/4", "5/16", "직접입력"]` 중 원클릭으로 신속 선택 가능.
       - **팝업 스크롤 오픈 시 깨끗한 공란 상태 유지 (`SelectField`)**: 옵션 목록 맨 첫 줄에 빈칸 옵션(`<option value="">`)을 상시 제공하여, 수치 초기화 상태에서 팝업 스크롤을 열었을 때 엉뚱한 수치(0 등)가 미리 선택되지 않고 완전한 공란에 스크롤/포커스가 머무르도록 완벽 개선.
       - **보정치 팝업 스크롤 기본값 명확화**: 보정치 드롭다운 첫 번째 항목을 `0 (보정 없음)`으로 단독 커스텀 지정하여 스크롤 열림 시 기본 상태를 직관적으로 확인 가능.
       - **오발 계산기 순차적 입력 흐름(Step-by-Step Locking) & 오발컷 자동 동기화**:
         * [수치 초기화] 시 피치 4축, 원홀, 오발, 컷 모두 공란으로 비워짐.
         * 원홀 미입력 시 오발크기/컷 잠금 ➔ 원홀 입력 시 오발크기 활성화 ➔ 오발크기 입력 시 오발컷1·2 활성화.
         * `오발컷 #1` 선택 시 `오발컷 #2`에 동일 값이 자동 동기화 입력되며, 이후 `#2` 단독 개별 수정 가능.
       - **오발컷 계산기로 돌아가 수치 변경 후 재진입 시 100% 자동 재연산**: 모달이 열릴 때마다 최신 입력 제원 스냅샷을 갱신하고, 변경된 제원의 $L_{\max}$에 따른 최적 추천 모드로 자동 즉시 세팅.
       - 리셋 시 하드코딩된 기본(3) 대신 **현재 제원 기준 초기 추천 모드로 완벽 복원**.
       - **덜컹거림(Layout Shift) 100% 완전 차단 & 선택 버튼 1:1 위치 일치**: 그라데이션을 걷어낸 **부드러운 파스텔톤 단색 플로팅 말풍선(`bg-amber-50 text-slate-900 border-amber-300`)**을 적용하고, 이모지(`⚠️`)를 제거하고 폰트 굵기를 `font-semibold`로 1단계 내려 깔끔하고 미니멀하게 정돈.
    6) **앱 재실행 시 마지막 모드(스판, 미드라인, 오발) 자동 복귀**:
       - `localStorage` 기반 마지막 작업 탭 실시간 동기화로 브라우저 새로고침 및 재실행 시 지공사님이 머무르셨던 화면으로 즉시 자동 복원.
  - **검증 및 배포 완료**: 70/70 전체 단위 테스트 PASS, Vite production build 성공 및 **Vercel 프로덕션 라이브 배포 완료 (`https://drilling-tools-app.vercel.app`)** 🚀.
