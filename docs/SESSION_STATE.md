...
- **신규 분기 브랜치 `드릴링-자동계산` [정밀도 3단계 (기본 3 / 정밀 5 / 초정밀 7) & 최대 8드릴(#8) 확장 & 실가공거리(L) 기반 자동추천 뱃지 및 부적격 경고 안내 연동 완성] (지공사님 지침에 따라 Vercel 배포는 보류 상태) (`OvalCalculatorView.jsx`, `OvalResultModal.jsx`, `Midline2DLayoutRenderer.jsx`, `SelectField.jsx`)**:
  - **지공사님 핵심 지침 100% 완수**:
    ...
    5) **실제 가공 이동거리($L_{\max}$) 기반 추천 뱃지 & 부적격 경고 배너 연동**:
       - **불필요한 과도 드릴링 방지 추천 기준 현실화**: $L_{\max} \le 0.0625"$ (편차 $1/8"$ 이하) ➔ **기본 3드릴 우선 추천**, $0.0625" < L_{\max} \le 0.125"$ (편차 $1/8" \sim 1/4"$) ➔ **정밀 5드릴**, $L_{\max} > 0.125"$ (편차 $1/4"$ 초과 롱 오발) ➔ **초정밀 7드릴**.
       - 모드 버튼 우상단에 **선명한 블랙 텍스트 16각 별모양(Starburst) 골드 추천 라벨 배지(`w-9.5 h-9.5`)**를 박스 외곽과 오버랩(`absolute -top-4 -right-3`)하여 입체감과 가독성을 극대화.
