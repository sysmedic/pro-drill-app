...
- **독립 PWA 웹 애플리케이션 'ProDrill Tools' [실제 오발선] 상시 기본 표출 & 이론적 오발선 영문 단일 스위치([ GUIDE LINE ]) 개편 완료 (지공사님 지침에 따라 Vercel 배포는 보류 상태) (`Midline2DLayoutRenderer.jsx`)**:
  - **지공사님 핵심 지침 100% 완수**:
    1) **골드 실제 오발선 상시 기본 표출**: 도면 기본 베이스 가이드라인으로 상시 100% 표출되며, 불필요해진 골드 실제 오발선 버튼은 전면 삭제.
    2) **영문 단일 스위치 `[ 🟢 GUIDE LINE ]` 배치**: `PREVIEW` 버튼 바로 밑에 영문 `GUIDE LINE` 단일 스위치를 배치하여, 스위치를 켰을 때만 초록 이론적 오발 타원선(0.6px #10b981)이 도면 위에 오버레이 표출.
    3) **원호(#1/#2) 보존 및 원홀 위치 안정성 100% 유지**: #1/#2의 고유 반경 원호가 실제 오발선과 1:1 완벽 밀착 접촉하며 안정적인 도면 렌더링 유지.
