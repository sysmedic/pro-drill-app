...
- **신규 분기 브랜치 `드릴링-자동계산` [시뮬레이터 좌측 `[ 🔴 리셋 ]` 칩 + 안전 경고 모달 + 드릴 추가 시 모드 전환 잠금] 탑재 완료 (지공사님 지침에 따라 Vercel 배포는 보류 상태) (`Midline2DLayoutRenderer.jsx`, `OvalResultModal.jsx`)**:
  - **지공사님 핵심 지침 100% 완수**:
    1) **시뮬레이터 좌측 `[ 🔴 리셋 ]` 칩 버튼 탑재**: 시뮬레이터와 동일한 반투명 글래스모피즘 디자인(`w-[130px] h-8`) 및 붉은 LED 램프를 풀스크린 및 인라인 도면 좌측 하단에 배치.
    2) **차트 최초 입력 제원 1:1 완벽 복원 & 안전 경고 인앱 모달**: 클릭 시 확인 경고 팝업을 띄우며, 승인 시 **원홀, #1, #2 비트 수치, 모든 이동 오프셋, 추가 비트를 차트 최초 계산 당시의 기본(3드릴 보통) 상태로 100% 롤백**.
    3) **추가 드릴 비트 존재 시 `[ 기본 / 정밀 ]` 스위치 잠금**: 드릴 비트가 추가된 상태(`extraBitCount > 0`)에서는 모드 토글이 잠기며, `[ 리셋 ]` 완료 후 자유롭게 정밀 모드 전환이 가능하도록 통제.
