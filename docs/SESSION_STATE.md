...
- **신규 분기 브랜치 `드릴링-자동계산` [도면 캔버스 내 리셋 버튼 소거 & 피치 매트릭스 헤더 단독 [ 🔴 리셋 ] 유지 및 안전 경고 모달 연동] 완료 (지공사님 지침에 따라 Vercel 배포는 보류 상태) (`Midline2DLayoutRenderer.jsx`, `OvalResultModal.jsx`)**:
  - **지공사님 핵심 지침 100% 완수**:
    1) **도면 캔버스 내 리셋 버튼 소거**: 블루프린트 캔버스(인라인 및 풀스크린)에서 불필요한 리셋 칩을 깨끗하게 삭제하고, `시뮬레이터` 칩만 단독 표출하여 도면 시야를 깔끔하게 확보.
    2) **오발 피치 매트릭스 상단 `[ 🔴 리셋 ]` 단독 배치**: `OVAL PITCH MATRIX` 헤더의 `시뮬레이터` 버튼 바로 좌측에 동일한 프리미엄 그라데이션 컨셉(`h-10 px-3.5 rounded-xl font-black`)으로 리셋 버튼을 유지.
    3) **차트 최초 입력 제원 1:1 완벽 롤백 & 안전 확인 인앱 모달**: 클릭 시 확인 경고 팝업을 띄우며, 승인 시 **원홀, #1, #2 비트 수치, 모든 이동 오프셋, 추가 비트를 차트 최초 계산 당시의 기본(3드릴 보통) 상태로 100% 롤백**.
    4) **추가 드릴 비트 존재 시 `[ 기본 / 정밀 ]` 스위치 잠금 및 리셋 후 전환 자유화**: 드릴 비트 추가 상태에서는 모드 토글이 잠기며, `[ 리셋 ]` 완료 후 자유롭게 정밀 모드 전환이 가능하도록 통제.
