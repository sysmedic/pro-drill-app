...
- 환경설정 및 로그아웃 모달 텍스트 1:1 단축, 이모지 100% 제거 및 가이드 뒤 배치 수술 완치 Vercel 배포 (`GeneralSettingsModal.jsx`, `CustomerHeader.jsx`):
  - **로그아웃 모달 정제**: 타이틀 이모지 `🚪` 100% 제거 (`계정 로그아웃`), 승인 버튼 문구 `로그아웃 승인` ➔ `로그아웃`으로 단축.
  - **환경설정 모달 이모지 및 텍스트 1:1 완치**:
    - 모달 메인 타이틀 이모지(`⚙️ 환경 설정`)만 남기고 내부 항목 이모지(`🔒`, `📁`, `📋`, `👤`) 100% 삭제.
    - `가이드 도움말 ?`: `?` 맥동 아이콘을 텍스트 뒤로 배치 교정.
    - 단축 텍스트 5종 및 설명 변경: `타임라인 로그`, `입력창 아코디언 박스`, `볼러스펙 카드`, `가이드 도움말`, `프로필`, 설명 `"테스크바에 지공사 프로필 이름 노출을 가립니다"`.
    - 아코디언 설명 문구 100% 삭제.
