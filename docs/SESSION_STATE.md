...
- **신규 분기 브랜치 `드릴링-자동계산` [0.001인치(0.0254mm) 단위 오발선 1:1 칼밀착(Tangency) 역산 엔진 완성] (지공사님 지침에 따라 Vercel 배포는 보류 상태) (`Midline2DLayoutRenderer.jsx`)**:
  - **지공사님 핵심 지침 100% 완수**:
    1) **0.001인치 단위 축선 정밀 스캔 엔진**: 내림(Floor) 결정된 비트 반경($R_{\text{fit}}$)의 외곽선이 초록색 실제 오발선 내벽과 1:1로 정확히 맞닿는 최적 축선 좌표($Y^*$)를 0.001" 단위로 역산.
    2) **오발선과의 틈새(Gap) 0단차 칼밀착**: 비트가 오발선 밖으로 0.001mm도 돌출되지 않으면서, 오발선과의 틈새를 0.001인치 이내로 완전 밀착(Tangency)시켜 사포질/줄질 부담을 극소화.
    3) **피치 매트릭스 상단 `[ 🔴 리셋 ]` 버튼 & 안전 확인 인앱 모달**: 원홀, #1, #2 비트 수치 및 모든 이동/추가 내역을 차트 최초 계산 당시 상태로 100% 롤백.
