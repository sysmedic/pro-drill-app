...
- **신규 분기 브랜치 `드릴링-자동계산` [정밀도 3단계 (기본 3 / 정밀 5 / 초정밀 7) & 최대 8드릴(#8) 확장 & 실가공거리(L) 기반 자동추천 뱃지 및 부적격 경고 안내 연동 완성] (지공사님 지침에 따라 Vercel 배포는 보류 상태) (`OvalCalculatorView.jsx`, `OvalResultModal.jsx`, `Midline2DLayoutRenderer.jsx`, `SelectField.jsx`)**:
  - **지공사님 핵심 지침 100% 완수**:
    ...
    5) **실제 가공 이동거리($L_{\max}$) 기반 추천 뱃지 & 부적격 경고 배너 연동**:
       ...
       - 모드 버튼 우상단에 **은은한 파스텔 골드 16각 별모양(Starburst) 추천 라벨 배지**를 박스 외곽과 오버랩(`absolute -top-3.5 -right-2.5`)하여 입체감 있고 차분하게 배치.
       - 리셋 시 하드코딩된 기본(3) 대신 **현재 제원 기준 초기 추천 모드로 완벽 복원**.
       - **덜컹거림(Layout Shift) 100% 완전 차단 & 선택 버튼 1:1 위치 일치**: 그라데이션을 걷어낸 **부드러운 파스텔톤 단색 플로팅 말풍선(`bg-amber-50 text-amber-900 border-amber-300`)**을 적용하고, 말풍선 꼬리 화살표가 선택된 버튼 위치(기본/정밀)에 1:1로 정밀 정렬되도록 안착.
