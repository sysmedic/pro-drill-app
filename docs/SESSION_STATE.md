...
- **신규 분기 브랜치 `드릴링-자동계산` [지공사님 정의 '미절삭 잔여 살(Gap) 스캔' 기반 AUTO FIT 엔진 완성] (지공사님 지침에 따라 Vercel 배포는 보류 상태) (`Midline2DLayoutRenderer.jsx`)**:
  - **지공사님 핵심 지침 100% 완수**:
    1) **미절삭 최대 공백(Uncut Void) 정밀 스캔 엔진**: 선택된 비트를 제외한 모든 기존 비트들의 절삭면과 초록색 실제 오발선 사이의 남은 살 두께($\text{Gap} = W_{\text{oval}} - W_{\text{cut}}$)를 100단계 정밀 스캔하여 잔여 공백이 가장 큰 정점($Y^*$)을 완벽히 포착.
    2) **오발 곡률 기반 최대 내접 직경(1/64" 무조건 내림) 및 최적 이동거리(Y*) 동시 연산**: 오발 외곽선 밖으로 0.001mm도 벗어나지 않는 안전한 표준 비트 규격과 1:1 완벽 밀착 좌표를 원클릭으로 계산 및 자동 안착.
    3) **3대 기준 홀(#1, #2, 원홀) 절대 잠금 안전장치 유지**: 오발 기준선 변형 100% 원천 차단.
