...
- **신규 분기 브랜치 `드릴링-자동계산` [현 위치 기반 최근접 빈 공간 1:1 칼밀착(Tangency) 오토핏 알고리즘 완성] (지공사님 지침에 따라 Vercel 배포는 보류 상태) (`Midline2DLayoutRenderer.jsx`)**:
  - **지공사님 핵심 지침 100% 완수**:
    1) **현 위치(currentAxialY) 기준 최근접 빈 공간 탐색**: 비트의 현재 위치를 중심으로 가중치 스코어링($\text{Score} = \text{Gap} \times 10 - \text{Dist} \times 2.5$)을 적용하여, 현재 위치와 가장 인접한 미절삭 빈 공간(오발선과의 틈이 큰 곳)을 정밀 포착.
    2) **최적 1/64" 내접 비트 결정**: 포착된 빈 공간 위치에서 오발선 밖으로 절대 돌출되지 않는 최대 비트 규격 자동 결정.
    3) **0.001인치(0.0254mm) 단위 1:1 칼밀착 스냅**: 해당 빈 공간의 축선 좌표로 0.001" 정밀도로 자석처럼 착! 밀착되어 오발선 좌우 내벽과의 단차/틈새 0 완전 밀착.
