...
export default function Midline2DLayoutRenderer({
  ...
  useEffect(() => {
    if (sharedState && sharedState.bitCustomSizes) {
      setBitCustomSizes(sharedState.bitCustomSizes);
    }
    if (sharedState && sharedState.bitCustomOffsets) {
      setBitCustomOffsets(sharedState.bitCustomOffsets);
    }
    if (sharedState && typeof sharedState.extraBitCount === 'number') {
  ...
  // 📌 D-Pad 이동 버튼 비활성 조건 (EDIT 모드 아님 / 선택 없음 / 1·2번 비트 / 마지막 비트는 이동 불가)
  const _dpadBaseDisabled =
    !isEditMode ||
    selectedBitIndex === null ||
    selectedBitIndex === 1 ||
    selectedBitIndex === 2 ||
    selectedBitIndex === totalActiveBits;
  const isSelectedBitHorizDisabled = _dpadBaseDisabled; // 좌우(◀/▶) 이동 버튼
  const isSelectedBitMoveDisabled  = _dpadBaseDisabled; // 상하(▲/▼) 이동 버튼
  ...
  useEffect(() => {
    const handleResize = () => {
      fullCanvasSizeRef.current = { w: 0, h: 0, dpr: 0 };
      canvasSizeRef.current = { w: 0, h: 0, dpr: 0 };
      requestDirectRender();
      // 화면 회전 직후 브라우저 렌더 트리 재배치 완료 시점 2차 보정
      requestAnimationFrame(() => {
        requestDirectRender();
      });
    };
  ...
  if (onlyFullScreenPortal) {
    ...
    return createPortal(
      <div
        ...
        className="fixed inset-0 z-[100] w-full h-[100vh] h-[100dvh] bg-slate-950 select-none overflow-hidden animate-fade-in"
        ...
        >
          ...
          {!isEditMode && (
            ...
            >
              <div
                ...
                className={`w-[165px] h-12 px-3.5 rounded-2xl flex items-center justify-between transition-all duration-150 backdrop-blur-md border shadow-2xl cursor-pointer ${
                  isCheckFillMode
          ...
          {isEditMode && (
            ...
            >
              ...
              {/* 📌 우측 상단 마스터 컨트롤 세트: 위 [ ✕ ] 닫기 / 아래 [ ◎ ] 실사이즈 보기 수직 배치 */}
          <div className="absolute top-3 right-3 flex flex-col items-center gap-1.5 z-20 select-none">
            {/* 1) [ ✕ ] 풀스크린 닫기 버튼 */}
            <button
              type="button"
            ...
            {/* 2) [ ◎ ] 실사이즈 보기 (첫 화면 오발 정밀 뷰) 버튼 */}
            <button
              type="button"
              onClick={setInitialOvalView}
              className="w-8 h-8 text-xs font-bold text-slate-300 bg-slate-900/90 hover:bg-slate-800/90 border border-slate-700/70 rounded-xl flex items-center justify-center cursor-pointer transition-colors shadow-md backdrop-blur-md active:scale-95"
              title="실사이즈 보기 (1:1 첫 화면 오발 정밀 뷰)"
            >
              ◎
            </button>
          </div>
        ...
        {!isEditMode && (
          ...
          >
            <div
              ...
              className={`w-[145px] h-10 px-3 rounded-2xl flex items-center justify-between transition-all duration-150 backdrop-blur-md border shadow-2xl cursor-pointer ${
                isCheckFillMode
                  ? 'bg-slate-900/95 text-white border-white/80 shadow-[0_0_15px_rgba(255,255,255,0.15)] font-black'
                  : 'bg-slate-900/90 text-slate-400 border-slate-700/80 hover:text-slate-200'
              }`}
